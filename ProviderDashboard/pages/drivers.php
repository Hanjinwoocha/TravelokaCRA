...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create') {
    ...
    $pdo->prepare("UPDATE driver_details SET drive_title=?,drive_firstname=?,drive_middlename=?,drive_lastname=?,drive_mobilenumber=? WHERE drive_id=?")
      ->execute([...$fields, intval($_POST['drive_id'])]);
    $msg = 'success:Driver updated.';
  } elseif ($action === 'delete') {
    $driveId = intval($_POST['drive_id']);
    // Block delete if assigned to any non-cancelled/non-completed order
    $cnt = $pdo->prepare("
      SELECT COUNT(*) FROM rental_order ro
      LEFT JOIN payment p   ON p.pay_rentid  = ro.rent_id
      LEFT JOIN eticket et  ON et.tick_payid = p.pay_id
      WHERE ro.rent_driveid = ?
        AND (et.tick_status IS NULL OR et.tick_status NOT IN ('Cancelled', 'Completed'))
    ");
    $cnt->execute([$driveId]);
    if ($cnt->fetchColumn() > 0) {
      $msg = 'error:Cannot remove this driver — they\'re assigned to one or more active or pending orders.';
    } else {
      $pdo->prepare("DELETE FROM driver_details WHERE drive_id=?")->execute([$driveId]);
      $msg = 'success:Driver removed.';
    }
  }
}

// Show drivers assigned to this provider's cars
$search = trim($_GET['q'] ?? '');
try {
  if ($search) {
    $stmt = $pdo->prepare("
      ...
      WHERE dd.drive_firstname LIKE ? OR dd.drive_lastname LIKE ? OR dd.drive_mobilenumber LIKE ?
      ORDER BY dd.drive_id DESC
    ");
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <div style="display:flex;align-items:center;gap:10px">
              <div style="width:32px;height:32px;border-radius:50%;background:var(--tv-blue-light);color:var(--tv-blue);display:flex;align-items:center;justify-content:center;font-weight:600;font-size:12px;flex-shrink:0">
                <?= strtoupper(substr($d['drive_firstname'],0,1).substr($d['drive_lastname'],0,1)) ?>
              </div>
              <strong><?= htmlspecialchars(trim(($d['drive_title']?$d['drive_title'].' ':'').$d['drive_firstname'].' '.($d['drive_middlename']?$d['drive_middlename'].' ':'').$d['drive_lastname'])) ?></strong>
            </div>
          </td>
          ...
          <td style="display:flex;gap:6px">
            ...
            <button class="btn-icon danger" onclick='confirmDelete("driver","del_drv_<?= $d['drive_id'] ?>",<?= json_encode(trim($d['drive_firstname'].' '.$d['drive_lastname']), JSON_HEX_APOS) ?>)'><i class="bi bi-trash3"></i></button>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
