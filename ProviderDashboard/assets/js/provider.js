...
// ── confirmDelete: supports both legacy (msg, formId) and admin-style (label, formId, name)
function confirmDelete(arg1, formId, entityName) {
  let title, msg;
  if (entityName !== undefined) {
    const cap = arg1.charAt(0).toUpperCase() + arg1.slice(1);
    title = 'Delete ' + cap + '?';
    msg   = 'You\'re about to permanently delete "' + entityName + '". This action cannot be undone.';
  } else {
    title = 'Are you sure?';
    msg   = arg1 || 'This action cannot be undone.';
  }
  tvConfirm({
    variant: 'danger',
    title, message: msg,
    confirmIcon: 'bi-trash3', confirmText: 'Delete',
...
// ── Intercept any form with data-confirm-message and route through tvConfirm
