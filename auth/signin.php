...
// Show message when redirected after mid-session deactivation
if (!empty($_GET['deactivated'])) {
    $error = 'deactivated:Your account has been deactivated. Please contact support for more information.';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'login') {
    ...
    if (!$email || !$pass) {
        ...
        // 2. Firebase Auth sign-in (covers both providers and customers)
        $auth = fb()->signIn($email, $pass);
        ...
        if (isset($auth['error'])) {
            ...
            // Check if this uid belongs to a vendor
            $vendors = fb()->query('vendors', [['field' => 'ownerId', 'op' => 'EQUAL', 'value' => $uid]]);
            if (!empty($vendors)) {
                $vendor = $vendors[0];
                ...
                if ($status === 'approved') {
                    ...
                    if (($vendor['isActive'] ?? true) === false) {
                        $reason = $vendor['deactivationReason'] ?? 'Your account has been deactivated by an administrator.';
                        $error  = 'deactivated:' . $reason;
                    } else {
                        unset($_SESSION['is_guest']);
                        $_SESSION['provider_logged_in']  = true;
                        $_SESSION['provider_id']         = $vendor['vendorId'] ?? $vendor['id'];
                        $_SESSION['provider_name']       = $vendor['businessName'] ?? '';
                        $_SESSION['provider_uid']        = $uid;
                        $_SESSION['provider_location']   = $vendor['locationName'] ?? '';
                ...
                // Check users collection for customer role
                $user = fb()->getDoc('users', $uid);
                if ($user && ($user['role'] ?? '') === 'customer') {
                    // Check deactivation
                    if (($user['isActive'] ?? true) === false) {
                        $reason = $user['deactivationReason'] ?? 'Your account has been deactivated by an administrator.';
                        $error  = 'deactivated:' . $reason;
                    } else {
                        unset($_SESSION['is_guest']);
                        $_SESSION['customer_logged_in'] = true;
                        $_SESSION['customer_id']        = $uid;
                        $_SESSION['customer_name']      = $user['fullName'] ?? $email;
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media (max-width: 860px) { .hero-panel { display:none; } .form-panel { padding:40px 32px; } }
    @media (max-width: 480px) { .form-panel { padding:32px 22px; } }
  </style>
</head>
<body>
<div class="card-wrap">
  <div class="hero-panel">
    ...
    <div class="hp-body">
      ...
      <div class="hp-role"><div class="hp-role-dot"><i class="bi bi-person"></i></div><div><div class="hp-role-title">Customers</div><div class="hp-role-sub">Find and book verified rental cars</div></div></div>
      <div class="hp-role"><div class="hp-role-dot"><i class="bi bi-building"></i></div><div><div class="hp-role-title">Providers</div><div class="hp-role-sub">Manage your fleet and incoming orders</div></div></div>
      <div class="hp-role"><div class="hp-role-dot"><i class="bi bi-shield-lock"></i></div><div><div class="hp-role-title">Admins</div><div class="hp-role-sub">Oversee the entire platform</div></div></div>
    </div>
  ...
  <div class="form-panel">
    ...
    <div class="alert-box alert-warn"><i class="bi bi-hourglass-split"></i><div><strong>Application under review</strong><br>Your provider application is pending admin approval.</div></div>
    <?php elseif ($error === 'rejected'): ?>
    <div class="alert-box alert-err"><i class="bi bi-x-circle-fill"></i><div><strong>Application not approved</strong><br>Your provider application was not approved. Contact support.</div></div>
    <?php elseif (str_starts_with($error, 'deactivated:')): ?>
    <?php $deactivatedReason = substr($error, strlen('deactivated:')); ?>
    <div class="alert-box alert-err"><i class="bi bi-slash-circle-fill"></i><div><strong>Account deactivated</strong><br><?= htmlspecialchars($deactivatedReason) ?><br><span style="font-size:11.5px;color:#9B1C1C;margin-top:4px;display:block">If you believe this is a mistake, please contact support.</span></div></div>
    <?php elseif ($error): ?>
    <div class="alert-box alert-err"><i class="bi bi-exclamation-circle-fill"></i><div><?= htmlspecialchars($error) ?></div></div>
    <?php endif; ?>
    ...
    <form method="post">
      ...
      <div class="f-mb">
        ...
        <div class="f-wrap"><i class="bi bi-envelope"></i>
      ...
      <div class="f-mb">
        ...
        <div class="f-wrap"><i class="bi bi-lock"></i>
    ...
    <div class="alt-links">
      <form method="post" style="margin:0">
        ...
        <button type="submit" class="alt-link">
          <span class="alt-link-l"><span class="alt-link-icon guest"><i class="bi bi-person-walking"></i></span><span><div class="alt-link-title">Continue as guest</div><div class="alt-link-sub">Browse and book — no account needed</div></span></span>
          <i class="bi bi-arrow-right alt-link-arr"></i>
        </button>
      </form>
      <a href="/Traveloka/auth/customer_login.php" class="alt-link">
        <span class="alt-link-l"><span class="alt-link-icon customer"><i class="bi bi-person-plus"></i></span><span><div class="alt-link-title">Create a customer account</div><div class="alt-link-sub">Save bookings &amp; unlock coupon codes</div></span></span>
        <i class="bi bi-arrow-right alt-link-arr"></i>
      </a>
    ...
    <p style="font-size:12.5px;color:var(--muted);text-align:center;margin-top:20px">
      <a href="/Traveloka/" style="color:var(--blue);font-weight:600;text-decoration:none"><i class="bi bi-arrow-left"></i> Back to home</a>
    </p>
  </div>
</div>
</body>
